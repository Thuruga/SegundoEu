...
from src.agents.sfx_generator import generate_sfx
from src.agents.video_editor import assemble_video

def build_graph():
    """
    Builds the LangGraph orchestration pipeline for Shortsophy.
    Flow: scriptwriter → voice_actor → media_researcher → sfx_generator → video_editor → END
    """
    ...
    workflow.add_node("sfx_generator",    generate_sfx)
    ...
    workflow.add_edge("media_researcher", "sfx_generator")
    workflow.add_edge("sfx_generator",    "video_editor")
